...
const navItems = [
  ...
  },
  {
    section: 'Operations',
    items: [
      { id: 'tools', label: 'Tool tracker', icon: 'tool' },
...
const icons = {
  ...
  currency: <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><circle cx="12" cy="12" r="10"/><path d="M16 8h-6a2 2 0 1 0 0 4h4a2 2 0 1 1 0 4H8"/><line x1="12" y1="6" x2="12" y2="8"/><line x1="12" y1="16" x2="12" y2="18"/></svg>,
}

export default function Sidebar({ activePage, onNavigate }) {
  const { data: toolsData } = useGet('/tools')
  const checkedOutCount = (toolsData || []).filter(t => t.status === 'out' || t.status === 'overdue').length
          ...
          <div key={group.section}>
            ...
            {group.items.map(item => (
              ...
              >
                ...
                {(item.badge || (item.id === 'tools' && checkedOutCount > 0)) && (
                  <span className="text-xs font-bold px-1.5 py-0.5 rounded-full text-white"
                    style={{ background: item.badgeColor === 'red' ? '#ef4444' : '#f59e0b', fontSize: '9px' }}>
                    {item.id === 'tools' ? checkedOutCount : item.badge}
                  </span>
                )}
              </button>
            ))}
